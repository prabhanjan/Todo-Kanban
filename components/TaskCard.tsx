    ...
    >
      <div className="flex justify-between items-start mb-2">
        ...
        <div className="relative">
          ...
          {showActions && (
            <div className="absolute right-0 top-6 z-10 bg-white border border-slate-200 shadow-lg rounded-md w-32 py-1 flex flex-col">
               <button
                onClick={() => {
                  onEnhance(task.id);
                ...
                className="text-left px-3 py-2 text-sm text-indigo-600 hover:bg-indigo-50 flex items-center gap-2 disabled:opacity-50"
              >
                <Wand2 size={14} />
      ...
      <p className="text-slate-500 text-sm mb-3 line-clamp-3">{task.description || "No description provided."}</p>

      <div className="flex flex-wrap gap-1 mb-3">
        {task.tags?.map((tag, idx) => (
          <span key={idx} className="px-2 py-0.5 bg-slate-100 text-slate-600 text-xs rounded-md">
            #{tag}
          </span>
        ))}
      </div>
      ...
      <div className="flex justify-between items-center text-xs text-slate-400 border-t border-slate-100 pt-2 mt-2">
        ...
        {isEnhancing && <span className="text-indigo-500 animate-pulse">AI Working...</span>}
      </div>
    </div>
  );
};
